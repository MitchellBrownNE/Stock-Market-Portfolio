...
# Function to insert overall sentiment data into PostgreSQL
def insert_overall_sentiment(overall_sentiment, final_score):
    conn = connect_db()
    cur = conn.cursor()
    insert_query = """
        INSERT INTO overall_sentiment_analysis (overall_sentiment, final_score)
        VALUES (%s, %s);
    """
    cur.execute(insert_query, (overall_sentiment, final_score))
    conn.commit()
    cur.close()
    conn.close()
...
def analyze_feed():
    ...
    if num_articles > 0:
        ...
        # Insert the overall sentiment into PostgreSQL
        insert_overall_sentiment(overall_sentiment, final_score)
