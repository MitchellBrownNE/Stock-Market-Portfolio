...
=======
  const [currentPrice, setCurrentPrice] = useState(null); // State for the current price
  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(false);
...
>>>>>>> Stashed changes
  ...
  const handlePredictedClick = async () => {
    ...
    try {
        // Fetch sentiment analysis data from Flask API
  ...
  const handlePredictedClick = () => {
    setModalOpen(true); // Open the modal when clicking predicted stock
  };
  ...
  return (
    <div className={`h-screen grid grid-cols-4 grid-rows-6 gap-4 p-10 font-quicksand relative ${theme === "dark" ? 'bg-bgdark' : 'bg-white'}`}>
<<<<<<< Updated upstream
=======
...
<<<<<<< Updated upstream
      <div className="col-span-1 row-span-1" onClick={() => handlePredictedClick(stockData.ticker)}>
    <Card>
